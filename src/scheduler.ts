...
export function createStudentSchedule(students:Student[],options:ScheduleOptions):ScheduleResult{
  ...
  let groups:Group[]=[...grouped].map(([code,list])=>{const r=refs.get(code)||[];const days=r.map(x=>x.previousDay).filter(Boolean) as string[],times=r.map(x=>x.previousTime).filter(Boolean) as string[];return{codes:[code],teacher:teacherFor(code,map),students:uniqueStudents(list),previousDay:days.sort((a,b)=>days.filter(x=>x===b).length-days.filter(x=>x===a).length)[0],previousTime:times.sort((a,b)=>times.filter(x=>x===b).length-times.filter(x=>x===a).length)[0],merged:false}});
...
export function createScheduleFromAi(students:Student[],options:ScheduleOptions,assignments:AiAssignment[],aiWarnings:string[]=[]):ScheduleResult{
  ...
  for(const proposal of assignments){
    ...
    const groupStudents=uniqueStudents(codes.flatMap(code=>studentsByCode.get(code)||[]));if(groupStudents.length>options.mergeMax&&codes.length>1){unplaced.push({codes,reason:"Birleşik grup öğrenci üst sınırını aşıyor"});continue}
