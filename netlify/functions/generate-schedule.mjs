...
export default async (request) => {
  ...
  try{
    ...
    let artResult;if(artStudents.length){const artInput=JSON.stringify({targetDay:"Öğrencinin ana günü veya istisna olarak Perşembe/Cuma",students:artStudents,lockedAssignments:baseAssignments,options:payload.options});if(artInput.length>900000)return json({error:"Müzik/Resim istisna verisi AI isteği için çok büyük."},413);artResult=selected==="anthropic"?await claude(artInput):await openAI(artInput)}const assignments=[...baseAssignments,...(Array.isArray(artResult?.parsed.assignments)?artResult.parsed.assignments:[])],warnings=[...baseWarnings,...(artStudents.length?[`${artStudents.length} Müzik/Resim öğrencisi, ana öğretmen saatleri kilitlenerek Perşembe/Cuma istisnasıyla ayrıca planlandı.`,...(Array.isArray(artResult?.parsed.warnings)?artResult.parsed.warnings:[])]:[])],first=baseResults[0]||artResult;return json({assignments,warnings,provider:first.provider,model:first.model});
