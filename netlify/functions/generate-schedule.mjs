...
export default async (request) => {
  ...
  try{
    ...
    const studentList=payload.students.slice(0,1500).map(student=>({...student,lessons:Array.isArray(student.lessons)?student.lessons.slice(0,2):[]})),regular=new Map([["Salı",[]],["Çarşamba",[]],["Cumartesi",[]]]),artStudents=[];for(const student of studentList){const color=String(student.color||"").toLocaleLowerCase("tr-TR"),grade=String(student.grade||"");if(student.artDayException){artStudents.push(student);continue}const targetDay=color==="turuncu"?"Cumartesi":grade.trim().startsWith("7")?"Salı":"Çarşamba";regular.get(targetDay).push(student)}
    ...
    let artResult;if(artStudents.length){const artInput=JSON.stringify({targetDay:"Öğrencinin ana günü veya istisna olarak Perşembe/Cuma",students:artStudents,lockedAssignments:baseAssignments,options:payload.options});if(artInput.length>900000)return json({error:"Müzik/Resim istisna verisi AI isteği için çok büyük."},413);artResult=selected==="anthropic"?await claude(artInput):await openAI(artInput)}let assignments=[...baseAssignments,...(Array.isArray(artResult?.parsed.assignments)?artResult.parsed.assignments:[])];const warnings=[...baseWarnings,...(artStudents.length?[`${artStudents.length} Müzik/Resim öğrencisi, ana öğretmen saatleri kilitlenerek Perşembe/Cuma istisnasıyla ayrıca planlandı.`,...(Array.isArray(artResult?.parsed.warnings)?artResult.parsed.warnings:[])]:[])];
    for(let repairAttempt=1;repairAttempt<=2;repairAttempt++){const covered=new Set(assignments.flatMap(assignment=>(assignment.studentIds||[]).flatMap(studentId=>(assignment.codes||[]).map(code=>`${studentId}|${String(code).toLocaleUpperCase("tr-TR")}`)))),missing=studentList.filter(student=>student.lessons.length>=2&&student.lessons.some(lesson=>!covered.has(`${student.id}|${String(lesson.code).toLocaleUpperCase("tr-TR")}`))).map(student=>({...student,missingLessonCodes:student.lessons.filter(lesson=>!covered.has(`${student.id}|${String(lesson.code).toLocaleUpperCase("tr-TR")}`)).map(lesson=>lesson.code)}));if(!missing.length)break;const repairInput=JSON.stringify({targetDay:"EKSİK ÖĞRENCİ TAMAMLAMA TURU",students:missing,lockedAssignments:assignments,options:payload.options,repairAttempt});if(repairInput.length>900000)break;const repairResult=selected==="anthropic"?await claude(repairInput):await openAI(repairInput),repaired=Array.isArray(repairResult.parsed.assignments)?repairResult.parsed.assignments:[];assignments=[...assignments,...repaired];warnings.push(`${missing.length} eksik öğrenci için ${repairAttempt}. otomatik tamamlama turu çalıştırıldı.`,...(Array.isArray(repairResult.parsed.warnings)?repairResult.parsed.warnings:[]));if(!repaired.length)break}
    const first=baseResults[0]||artResult;return json({assignments,warnings,provider:first.provider,model:first.model});
